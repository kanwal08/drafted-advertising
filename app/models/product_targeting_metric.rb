...
#  product_targeting_id          :bigint           not null
#  date                          :date
#  impressions                   :integer
#  clicks                        :integer
#  cost                          :decimal(, )
...
class ProductTargetingMetric < ApplicationRecord
  ...
  def self.create_from_report(record, profile)
    product_targeting = ProductTargeting.find_by(product_targeting_external_id: record['keywordId'])

    unless product_targeting
      ...
      product_targeting = ProductTargeting.create!(product_targeting_external_id: record['keywordId'], ad_group: ad_group)
    end
    
    metric = find_or_create_by(date: record["date"], product_targeting: product_targeting)
